...
//        }
                ...
                if ($data_valid === true) {
                    ...
                    if ($item['Wheel Tyre Accessory'] == 'Tyre') {
                        if((string)$item['Include in Price Match']=='True'){
                            ...
                            $rsp_price = round($this->get_rsp(
                                $item,
                                $product_id,
                                (string)$item['Cost Price'],
                                $is_variable ? (float)wc_get_product($children[0])->get_regular_price() : (float)$product->get_regular_price()
                            ) * 1.2,2);
                        }else{
    ...
    private function get_rsp($item, $product_id, $price, $fallback_price)
    {
        //First check that price is not zero
        if($price == 0)
        ...
        //}
        if($pc){
            $rsp_price =  (($pc/100) * $s_price) + $s_price + $this->flat_fee;
            return $rsp_price > 40 ? $rsp_price : $fallback_price;
        }else{
            return $price;
        }
    }
